          ...
          <div class="modal-body">
            ...
            <div class="form-group">
                <label>Pilih Direktorat</label>
                  <select class="form-control" name="role" id="edit_direktorat_id" style="width: 100%">
                    @foreach ($direktorats as $direktorat)
                        @if (old('direktorat_id', $direktorat->nama) == $direktorat->id)
                        <option value="{{ $direktorat->id }}" selected>{{ $direktorat->nama }}</option>
                        @else
